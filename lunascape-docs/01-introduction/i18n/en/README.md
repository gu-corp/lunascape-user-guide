...
| Translate | Generate translation proposals per page or in batch, then review before saving <!-- ai-only --> |
| Use from AI | A read-only specification tool that VS Code agents can consult <!-- ai-only --> |
...
- **AI only proposes.** Translation proposals are saved after you review the diff. Documents are never rewritten silently. <!-- ai-only -->
